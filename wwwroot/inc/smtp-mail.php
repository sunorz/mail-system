...
class ToolUtils {
	...
	protected $_key = "jKfm4T5890KwYRf459WE03nu8546";
	//加密
    ...
    //发送邮件
	public function sendmail($fr,$to,$type,$ct,$at,$sub)
	{
	...
	**/
	$recv=array();
	$query2=mysql_query("select csmail from custinfo where cscate=".$to." and csflag=0");
	//拷贝数组，并且清除下标
	...
	**/
	$mail = new MySendMail();
	$mail->setServer("smtp.maildomain",$sender,$pwd,465,true); //设置smtp服务器，普通连接方式
	$mail->setFrom($sender); //设置发件人
	/**
	* 发送之前，将邮件内容$ct套入模板$template
	**/
	//$this->setLan('ja_JP','smtp-mail');//设置本地化
	...
	if($type==0){
		...
		$mail->setReceiver($recv[$i]); //设置收件人，多个收件人，调用多次
		sleep(10);
		$mail->sendMail(); //发送
		}
	...
	{
		//BCC
		$mail->setReceiver("info@maildomain"); //只是发送给info
		$cta=$this->createTemp(0,$ct);//套入模板
	    $mail->setMail($sub, $cta); 
		for($j=0;$j<count($recv);$j++)
		{
		$mail->setBcc($recv[$i]); //设置秘密抄送，多个秘密抄送，调用多次
		}
		$mail->sendMail(); //发送
	}	
	return true;
	...
	//本地化模板
	public function setLan($lang,$local){
     putenv('LANG='.$lang);   
     setlocale(LC_ALL,$lang);
     bindtextdomain ($local,dirname(__FILE__).'\locale'); //设置某个域的mo文件路径    
     bind_textdomain_codeset($local,'UTF-8');  //设置mo文件的编码为UTF-8    
     textdomain($local);                    //设置gettext()函数从哪个域去找mo文件  
	...
	public function createTemp($mail,$content)
	{
		...
		if($mail!=0){
			...
			$str='</div>
<div id="jfooter"><a class="ursub" href="http://maildomain/inc/unsubcribe.php?token='.$token.'">'.strtoupper('unsubscribe').'</a></div>	
</div>';
		}
	...
	#jwrap{
		...
		margin: 0 auto;
		background: #6cceee;
		border: #6cceee solid 5px;
		border-radius: 5px;
		
	}
	#jtitle{color: aliceblue;background:#6cceee;top:0;font-size:2em;font-weight: bolder;padding: 0.5em 0 0.5em 0.5em;}
	#jcontent{background:#f8efc4;padding: 10px;}
	
	.ursub{color:aliceblue;font-size: 0.7em;text-decoration: none;}
	.ursub:hover{color:yellow;}
	#jfooter{text-align: right;}
</style><div id="jwrap">
<div id="jtitle">Jasgo Traslate</div>
<div id="jcontent">'.$content.$str;
	}
